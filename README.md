...
| `SCAN_FILENAME_TEMPLATE` | `SCAN_${date}__${uid}` | Filename template (without extension). Supported substitutions: `${date}` → `YYYY-mm-dd`, `${datetime}` -> `YYYY-mm-dd HH:MM:SS`, `${uid}` → zero-padded sequence number, `${homedir}` → home directory of `OWNER`. When providing a value via docker-compose.yml remember to escape the `$` by using `$$` |
